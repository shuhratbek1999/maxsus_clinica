          ...
          let date =Math.floor(new Date().getTime() / 1000);
           ...
           setTimeout(async() => {
            ...
            if(tolov != null){
                ...
                if(have==undefined){
                    this.q.push({"room_id":user.room_id,"patient_id":model.patient_id,"number":0,"date_time":Math.floor(new Date().getTime() / 1000),"status":model.status});
                }
                else if(element.status!=have.status){
                    if(element.status!='complate'){
